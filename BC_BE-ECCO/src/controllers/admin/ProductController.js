  ...
  try {
    ...
    const results = await Promise.all(
      products.map(async (item) => {
        ...
        const totalQuantityByProduct = order_products.reduce((acc, item) => {
        ...
        return {
          ...
          total_quantity: totalQuantityByProduct[item.Id] || 0
...
const storeProduct = async (req, res) => {
  try {
    upload.single("image")(req, res, async (err) => {
      const { name, price, quantity, category_id, description,sale = 0 } = req.body;
      let image = "";
      if (req.file) {
...
const updateProduct = (req, res) => {
  try {
    upload.single("image")(req, res, async (err) => {
      ...
      const { name, price, quantity, category_id, description, product_id,sales= 0 } =
        req.body;
      let image = "";
      if (req.file) {
